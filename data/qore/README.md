...
The first Arctic Blast baseline strategy tests are generated by:

```bash
npm run optimize:arctic-strategies
```

The command writes ten variants under `research/strategy-tests/`: five `strict-theory` strategy tests that only trade score-qualified Arctic Blast rows in the `theory.md` direction, and five `experimental` tests that keep the wider search space.
