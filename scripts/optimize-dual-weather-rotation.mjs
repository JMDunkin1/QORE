...
const OUTPUT_DIR = path.join(DATA_ROOT, 'research/strategy-agent-runs/dual-weather-rotation')
...
const STRATEGY_ID = 'dual-weather-rotation'
...
function createSignalFromIssueRows(rows, candidate, reliabilityWeights) {
  ...
  const winner =
    cold.maxStrength >= warm.maxStrength
      ? { side: 'cold', direction: 1, stats: cold, loser: warm }
      : { side: 'warm', direction: -1, stats: warm, loser: cold }
...
function summarizeCandidate(days, rowsByIssueDate, indexBenchmarks, candidate, reliabilityWeights, signalCache, options = {}) {
  ...
  const result = {
    ...
    architectureId: 'dual-weather-follow-and-fade',
...
function isEligible(result) {
  ...
  return (
    ...
    trainValidationLegs.coldLong >= 4 &&
    trainValidationLegs.warmShort >= 4 &&
    ...
    trainValidationSides.coldLong.totalReturnPct > 0 &&
    trainValidationSides.warmShort.totalReturnPct > 0 &&
...
function trainValidationRank(result) {
  ...
  const sideQuality =
    Math.min(10, result.sideReturns.trainValidation.coldLong.totalReturnPct) +
    Math.min(10, result.sideReturns.trainValidation.warmShort.totalReturnPct) +
...
function formatCandidateRow(candidate) {
  return {
    ...
    coldLongRows: candidate.legCounts.all.coldLong,
    warmShortRows: candidate.legCounts.all.warmShort,
...
function buildReport(summary) {
  ...
  return `# Dual Weather Rotation Lane
...
- Weather leg: ${selected.weatherFraction}x max UNG overlay for ${selected.followHoldDays} trading day(s), long for broad cold and short for broad warmth.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Train/validation | ${selected.legCounts.trainValidation.weatherFollow} | ${selected.legCounts.trainValidation.weatherReversion} | ${selected.legCounts.trainValidation.coldLong} | ${selected.legCounts.trainValidation.warmShort} | ${selected.legCounts.trainValidation.reversionLong} | ${selected.legCounts.trainValidation.reversionShort} |
| Holdout | ${selected.legCounts.holdout.weatherFollow} | ${selected.legCounts.holdout.weatherReversion} | ${selected.legCounts.holdout.coldLong} | ${selected.legCounts.holdout.warmShort} | ${selected.legCounts.holdout.reversionLong} | ${selected.legCounts.holdout.reversionShort} |
...
| --- | ---: | ---: | ---: | ---: |
| Cold-long | ${selected.sideReturns.trainValidation.coldLong.tradeCount} | ${selected.sideReturns.trainValidation.coldLong.totalReturnPct}% | ${selected.sideReturns.trainValidation.coldLong.sharpe} | ${selected.sideReturns.trainValidation.coldLong.maxDrawdownPct}% |
| Warm-short | ${selected.sideReturns.trainValidation.warmShort.tradeCount} | ${selected.sideReturns.trainValidation.warmShort.totalReturnPct}% | ${selected.sideReturns.trainValidation.warmShort.sharpe} | ${selected.sideReturns.trainValidation.warmShort.maxDrawdownPct}% |
...
function main() {
  ...
  writeText(path.join(OUTPUT_DIR, 'candidate-summary.csv'), rowsToCsv(summaryCandidates, [
    ...
    'reversionLongRows',
    'reversionShortRows',
    'trainValidationColdLongReturnPct',
    'trainValidationWarmShortReturnPct',
